...
def test_leaf_split_links_leaf_chain_in_order(tmp_path) -> None:
    ...
    assert tree.range(None, None) == list(range(200))
...
def test_multi_level_tree_still_answers_correctly(tmp_path) -> None:
    ...
    assert tree.lookup(599) == [599]
    assert tree.lookup(-1) == []
    assert tree.range(100, 109) == list(range(100, 110))
...
def test_lookup_returns_all_duplicates_spanning_two_leaves(tmp_path) -> None:
    ...
    assert tree.lookup(7) == list(range(250))
...
def test_range_open_bounds_exclude_endpoints(tmp_path) -> None:
    ...
    assert tree.range(3, 6, lower_inclusive=False) == [4, 5, 6]
    assert tree.range(3, 6, upper_inclusive=False) == [3, 4, 5]
    assert tree.range(3, 6, lower_inclusive=False, upper_inclusive=False) == [4, 5]
...
def test_range_supports_unbounded_sides(tmp_path) -> None:
    ...
    assert tree.range(None, 3) == [0, 1, 2, 3]
    assert tree.range(7, None) == [7, 8, 9]
    assert tree.range(None, None) == list(range(10))
...
def test_range_lower_greater_than_upper_is_empty(tmp_path) -> None:
    ...
    assert tree.range(8, 2) == []
    assert tree.range(4, 4, lower_inclusive=False) == []
...
def test_range_crosses_leaf_boundary(tmp_path) -> None:
    ...
    assert tree.range(150, 199) == list(range(150, 200))
...
def test_range_includes_duplicates_at_bounds(tmp_path) -> None:
    ...
    assert tree.range(5, 5) == list(range(120))
    assert tree.range(5, 6) == list(range(120)) + [999]
...
def test_int_keys_order_includes_negatives(tmp_path) -> None:
    ...
    assert tree.range(None, None) == [1, 2, 3, 4]
...
def test_real_int_probe_is_normalized_to_float(tmp_path) -> None:
    ...
    assert tree.lookup(2) == [4]


def test_text_prefix_sorts_before_longer_key(tmp_path) -> None:
    tree, _, _, _ = _make(tmp_path, SqlType.TEXT)
    for key in ("ab", "a", "b"):
        tree.insert(key, len(key))

    assert tree.range(None, None) == [1, 2, 1]
...
def test_boolean_keys_support_equality(tmp_path) -> None:
    ...
    assert tree.lookup(True) == [2]
    assert tree.lookup(False) == [1]


def test_key_type_mismatch_raises_type_mismatch(tmp_path) -> None:
    tree, _, _, _ = _make(tmp_path, SqlType.INT)

    _expect_code(lambda: tree.lookup("x"), E_TYPE_MISMATCH)
    _expect_code(lambda: tree.insert(1.5, 1), E_TYPE_MISMATCH)
...
def test_split_reuses_page_returned_to_free_list(tmp_path) -> None:
    ...
    assert tree.range(None, None) == list(range(250))
...
def test_tree_survives_flush_and_reopen(tmp_path) -> None:
    ...
    assert reopened.lookup(123) == [123]
    assert reopened.range(10, 12) == [10, 11, 12]
...
def test_randomized_model_matches_sorted_reference(tmp_path) -> None:
    ...
    reference.sort()
    assert tree.range(None, None) == [rid for _, rid in reference]
    for key in range(60):
        assert tree.lookup(key) == sorted(
