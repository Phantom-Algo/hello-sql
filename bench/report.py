...
def render_markdown(result: dict[str, object]) -> str:
    ...
    lines: list[str] = [
        ...
        "所以那一列对 SQL 行只作参考。",
        "- 冷启动的**第一次顺序扫描**还要付一次 D37 惰性布局基线（逐页读一遍），"
        "因此页读约为数据页数的 2 倍；这是 B 的既有机制，两种 seq 模式同等承担。",
    ...
    lines.extend(
        [
            ...
            "## 已知限制",
            "",
            "- **冷启动回表**：B 在 rid→页 映射未建立时逐页探测（`engine._locate` 的退化分支），"
            "所以冷进程里索引点查/区间扫描的成本被放大；同进程第二次起才 O(1)。"
            "B 侧登记为 D49（回表 O(1)）待排期。",
