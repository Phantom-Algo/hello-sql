...
- **存储模式不预热也不经过规划器**，报的是纯 B 成本；D49a 起索引叶条目自带
  行页号，冷启动的回表也是每行一页（页号只是提示，读侧校验不符会退回全表定位）。
...
- 插入页提示（D49c）：装载 4000 行的页读从约 34/行降到约 2/行，数据集构建时间
  从 ~7s 降到 ~1.4s。
- 回表 O(1)（D49a）：索引叶条目自带行页号，冷进程的 `api-index` 与
  `--physical index` 从"每行读满数据页"变成"每行读一页"（99 行区间 4724 →
  129 页读）；索引文件版本升到 2，旧目录里的 v1 索引由数据集准备阶段触发一次
  自动重建，不污染测量窗口。
