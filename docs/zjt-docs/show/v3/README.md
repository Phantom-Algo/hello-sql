...
| 回表定位 | **已修复**（B 的 D49a）：索引叶条目携带行所在页号，冷进程回表直接读那一页 | 无需预热统计或建映射；页号只是提示，读侧校验不符会退回全表定位 |
| 索引文件版本 | `INDEX_FILE_VERSION = 2`（D49a 起叶条目多一个 `u32` 页号） | 旧目录里的 v1 索引在首次被访问时自动重建（一次 O(n log n)） |
