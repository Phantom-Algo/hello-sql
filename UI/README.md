...
- Parser：真实规则调用顺序、Token 消费和返回节点；事件使用
  “构建 SELECT”“解析投影列表”等职责说明，而不仅显示函数名与递归深度；
- AST：不可变 dataclass 树的节点类型、字段和路径，同时说明节点中的
  表、列、运算符、字面量或索引目标；
- SourceSpan：语句原文在完整脚本中的一基闭区间，事件直接显示具体起止行列。

AST 追踪按 dataclass 通用遍历，不使用语句类型白名单。V3 的
`CreateIndexStmt` 和 `DropIndexStmt` 会自动进入同一个 `a.ast` 阶段；在
`/inspect A` 的“节点”面板中分别显示“索引名 → 表.列”和“索引名”摘要，
...
`127.0.0.1` 随机端口的本地服务，使用系统默认浏览器打开蓝色科技风查看器。
首屏默认进入“节点”树形视图，只展示查询原文、阶段摘要以及抽象语法树、计划和执行树
主结构。阶段事件、节点/Token/页摘要及关联关系继续以可视化卡片展示；原始
JSON 和输入/输出快照默认完全隐藏，只有点击页面顶部的“原始数据”开关才会
统一显示，再次点击即可关闭。该开关只改变浏览器显示状态，不会删除追踪数据、
重新解析或重新执行 SQL。Parser 阶段默认仅展示主要文法步骤；
`advance`、`expect` 与标识符规范化等底层动作仍保留真实调用顺序，可通过
“展开底层步骤”按钮查看；递归深度和 Token 消费数作为悬停技术信息，不再占据主说明。
桌面端 Pipeline 会按视口高度自动伸缩，阶段超过可见
区域后可直接用鼠标滚轮独立滚动，不再由十四个阶段撑长右侧区域；窄屏下恢复为
双列或单列的自适应列表。纯文本/管道模式不弹窗，只输出无 ANSI 的稳定摘要。
...
查看器右侧增加“阶段 / 节点 / 词法单元 / 数据页”四个面板。可以：
