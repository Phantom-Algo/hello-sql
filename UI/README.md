...
首屏默认进入 NODES 树形视图，只展示查询原文、阶段摘要和 AST/Plan/Executor
主结构。阶段事件、节点/Token/页摘要及关联关系继续以可视化卡片展示；原始
JSON 和输入/输出快照默认完全隐藏，只有点击页面顶部的 `RAW DATA` 开关才会
统一显示，再次点击即可关闭。该开关只改变浏览器显示状态，不会删除追踪数据、
重新解析或重新执行 SQL。桌面端 Pipeline 会按视口高度自动伸缩，阶段超过可见
...
5. 使用 `RESET LINK` 清除选择、高亮和已经展开的对象详情。
